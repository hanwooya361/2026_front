...
let grade1 = Number(prompt('점수1: '))      // 1.입력 2. 저장
let grade2 = Number(prompt('점수2: '))
let totalgrade = grade1 + grade2            // 3. 처리
if(totalgrade >= 90){console.log('성공')}             //4. 출력 (분기/조건) , 총점이 90점이상
else{console.log('실패')} */
...
// 조건: 첫번째 값이 두번째 값보다 더 크면, 참: 첫번째 값 , 거짓 : 두번째 값
if(firstnum > secondnum) {console.log('fistnum')}
...
12자 이상: '보안 등급: 강함' 
let pw = prompt('비밀번호: ')
if(pw.length >= 12) {console.log('보안등급:강함')}
else if(pw.length >= 8 ) {console.log('보안등급:보통')}
...
let seatList = [ 'O', 'X', 'O' ];
let selectnum = Number(prompt('좌석번호: 0~2 '))    // 1. 입력받기 2. 저장하기
let result = seatList[selectnum] == 'X'     // 3. 처리
if(result) {console.log('예약 성공')}       // 4. 출력받기
...
let gamescore = parseInt(prompt('게임 점수: '))
if(gamescore >= 900) {console.log('A급 경품')}
else if(gamescore >= 700 ) {console.log('B급 경품')}
else if(gamescore >= 500) {console.log('C급 경품')}
...
let userrole = prompt('사용자 역할: ')
if(userrole=='admin') {console.log('모든 기능에 접근할 수 있습니다.')}
else if(userrole=='editor') {console.log('콘텐츠 수정 및 생성 기능에 접근할 수 있습니다.')}
else if(userrole=='viewer') {console.log('콘텐츠 조회만 가능합니다.')}
...
let age = parseInt(prompt('사용자 나이: '))
if(age < 8) {console.log('무료')}
else if(age >= 8 & age <= 19) {console.log('5000원')}
else if(age >= 20 & age < 65) {console.log('10000원')}
else{console.log('3000원')} */
...
drinkPrices = [1000, 1000, 1500];
let user = Number(prompt('0:콜라 , 1:사이다 , 2:커피'))
if(user < 0 || user > 2) {
    console.log('없는 상품입니다')
}else{
    console.log(`선택하신 음료는 ${drinkNames[user]} 입니다. 가격은 ${drinkPrices[user]}원 입니다)
} */
