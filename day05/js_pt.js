...
let numbers = [23, 5, 67, 12, 88, 34];
let bs = numbers[0];
for(let bsindex = 1; bsindex <= numbers.length-1; bsindex++){
    if(numbers[bsindex] > bs){
        bs = numbers[bsindex]
    }
}
console.log(bs)
...
let numbers = [23, 5, 67, 12, 88, 34];
// 반복문 없이 풀리
let max = number[o];    //배열명[인덱스] : 특정 인덱스 번째 요소 값 호출 // 첫번째 값을 최대값으로 지정
if( max < numbers[1]){ max = numbers[1]}    // 만약 [1] 요소 값이 더 크면 최대값 지정
...
*       line=5     star = 1
// line은 1~5까지 1씩 증가하면 '\n' 출력
// star은 1부터 (마지막줄수 - 현재줄수 +1) 1씩증가하면서 '*' 출력
// 줄 마다 별 출력? 별 마다 줄 출력? 전자
...
for(let i = 0; i <= userNames.length-1; i++){
    console.log(userNames[i])
    if(userNames[i].indexOf('솔) !== -1){        //.indexOf(찾을값) : 찾을값이 존재하면 인덱스 반환 , 없으면 -1 반환
    console.log(userNames[i])
    {break}
    }           
}
...
let seatLayout = [['A1', 'A2', 'A3'], ['B1', 'B2', 'B3'], ['C1', 'C2', 'C3']];
// 1. 행 먼저 반복문 이용하여 조회. 
for(let row = 0; row <= seatLayout.length-1; row++){    // 0행~2행까지 1씩 증가하면서 행 출력
    for(let col = 0; col <= seatLayout[row].length-1; col++){       // 0열~2열까지 1씩 증가하면서 열 출력
...
교수님:
let numbers = [5, 3, 4, 1, 2]; // 5>3, 5>4, 5>1, 5>2/ 3>4 3>1 3>2/ 4>1, 4>2/ 1>2
// 비교기준(인덱스) : 0     1       2       3
// 비교대상:        1234   234     34       4   , 비교기준 +1부터 마지막인덱스까지
for(let i = 0; i < numbers.lengh-2; i++){   // i는 0부터 마지막인덱스 전까지
    for(let j = i+1; j <= numbers.length-1; j++){   //j는 i+1부터 마지막인덱스 까지
        if(numbers[i] > numbers[j]) {
            let temp = numbers[i]   // 스왑(교체)   ,   i=5, j=3, temp=5
            numbers[i] = numbers[j] // i=3, j=3, temp=5
            numbers[j] = temp;      // i=3, j=5, temp=5
        }
    }
}
...
for(let i =0; i <= carNumbers.length-1; i++){
   ...
   if(usageMinutes[i] > 30){
    let ef = parseInt((usageMinutes[i] - 30) / 10)
    fee += ef * 500
   }
   if(fee > 20000) {
    fee = 20000
   }
   html += carNumbers[i] + ':' + usageMinutes[i] + '분 주차 , 최종 요금 :' + fee + '원 <br>'
}
document.querySelector('p').innerHTML = html
