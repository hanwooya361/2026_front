...
/*
    ...
    2. 메모리 설계
        ...
        2) 속성들 간의 종속관계(1:n, n:1) 파악, 테이블 나누기
            카테고리명  제품명  가격    이미지      등록일
            음료        콜라    1000    xxx.png     2026-1-11
            음료        사이다  1000    xxx.png     2026-1-12
        --------------------------------------------------------

        3). 쪼개진 테이블 간 연관 만들기, 관계형DB
            3-1) 테이블당 식별자(PK = PREMARY KEY)1개 권장
            3-2) 상하관계 파악해 식별자(코드) 하위요소에 저장한다. (FK : 참조/외래 키) , 교집합
            카테고리번호    카테고리명  제품코드    제품명      가격        이미지      등록일          카테고리코드(FK)
            1               음료        1           콜라        2500        XXX.PNG     2026-1-11           1
            2               과자        2           환타        2000        XXX.PNG     2026-1-12           1
        4) JS(JSON)로 표현 , 표 = 배열 / 객체 = 행 / 열 = 속성

    3. 기능 설계 : CRUD , RESTAPI
        등록(CREATE)    : 게시물쓰기, 회원기입, 수강신청 등등
        ...
        삭제(DELETE)    : 게시물삭제, 회원탈퇴 등등
            실행조건    : 삭제버튼 클릭했을때, 함수명: productDelete, 매개변수 : 삭제할제품코드(누구를/대상), 반환값:x

*/
// 2.
let categoryList = [ {'cocode':1, 'cname':'음료'} , {'cocode':2, 'cname':'과자'} ]
let productList = [
    {'pcode': 1, 'pname': '콜라', 'pprice': 1000, 'pimg' : "https://placehold.co/100x100", 'pdate' : '2026-07-22', 'ccode' : 1 },
    {'pcode': 2, 'pname': '사이다', 'pprice': 1000, 'pimg' : "https://placehold.co/100x100", 'pdate' : '2026-07-23', 'ccode' : 1 },
]
...
function productPrint(){
    ...
    for(let i=0; i<productList.length; i++){
        ...
        for(let j=0; j<categoryList.length; j++){
            if(categoryList[j].code == product.code){   // 만약 카테고리내 카테고리번호 와 제품의 카테고리번호가 같다면
                cname = categoryList[j].cname;
                break;  // 찾았으면 종료
            }
        } 
        // 만일 샘플 html 존재하면 복사해서 ` 백틱 사이 복붙 , 변수 자리에 ${ } 변경
        html += `<tr>
...
function productAdd(){
    ...
    const img = document.querySelector('.imgage').value
